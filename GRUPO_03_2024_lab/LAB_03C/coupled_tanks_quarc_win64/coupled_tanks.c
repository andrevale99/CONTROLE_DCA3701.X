 ...
 * C source code generated on : Fri Jan 10 16:09:21 2025
 *
...
void coupled_tanks_output(void)
{
   ...
   */
  coupled_tanks_B.Sinal_nao_Seguro = (coupled_tanks_P.K2[0] *
    coupled_tanks_X.Integrator1_CSTATE[0] + coupled_tanks_P.K2[1] *
    coupled_tanks_X.Integrator1_CSTATE[1]) + coupled_tanks_P.K1 *
    coupled_tanks_X.Integrator_CSTATE;
  ...
  switch ((int32_T)((coupled_tanks_P.OpNormal_Value - (real_T)
                     (coupled_tanks_B.Compare ? coupled_tanks_B.Sinal_nao_Seguro
                      < coupled_tanks_P.Constant_Value_k : 0)) - (real_T)
                    (coupled_tanks_B.Sinal_nao_Seguro >=
   ...
   default:
    ...
    if (coupled_tanks_B.Sinal_nao_Seguro > coupled_tanks_P.Saturation_UpperSat)
    {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_P.Saturation_UpperSat;
    } else if (coupled_tanks_B.Sinal_nao_Seguro <
               coupled_tanks_P.Saturation_LowerSat) {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_P.Saturation_LowerSat;
    } else {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_B.Sinal_nao_Seguro;
    }
   ...
   *  Integrator: '<Root>/Integrator1'
   */
  rtb_Sum3 = coupled_tanks_B.VoltstocmTank2 - (coupled_tanks_P.C[0] *
    coupled_tanks_X.Integrator1_CSTATE[0] + coupled_tanks_P.C[1] *
    coupled_tanks_X.Integrator1_CSTATE[1]);
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
   ...
   *  Integrator: '<Root>/Integrator1'
   */
  coupled_tanks_B.x[0] = (coupled_tanks_P.L[0] * rtb_Sum3 + coupled_tanks_P.B[0]
    * coupled_tanks_B.MultiportSwitch) + (coupled_tanks_P.A[0] *
    coupled_tanks_X.Integrator1_CSTATE[0] + coupled_tanks_P.A[2] *
    coupled_tanks_X.Integrator1_CSTATE[1]);
  coupled_tanks_B.x[1] = (coupled_tanks_P.L[1] * rtb_Sum3 + coupled_tanks_P.B[1]
    * coupled_tanks_B.MultiportSwitch) + (coupled_tanks_P.A[1] *
    coupled_tanks_X.Integrator1_CSTATE[0] + coupled_tanks_P.A[3] *
    coupled_tanks_X.Integrator1_CSTATE[1]);
}
...
void coupled_tanks_derivatives(void)
{
  ...
  _rtXdot->Integrator1_CSTATE[0] = coupled_tanks_B.x[0];
  _rtXdot->Integrator1_CSTATE[1] = coupled_tanks_B.x[1];
}
...
void coupled_tanks_initialize(void)
{
  ...
  coupled_tanks_X.Integrator1_CSTATE[0] = coupled_tanks_P.Integrator1_IC[0];
  coupled_tanks_X.Integrator1_CSTATE[1] = coupled_tanks_P.Integrator1_IC[1];
}
...
RT_MODEL_coupled_tanks_T *coupled_tanks(void)
{
  ...
  /* External mode info */
  coupled_tanks_M->Sizes.checksums[0] = (2212827727U);
  coupled_tanks_M->Sizes.checksums[1] = (3241020222U);
  coupled_tanks_M->Sizes.checksums[2] = (2656461265U);
  coupled_tanks_M->Sizes.checksums[3] = (2209958701U);
  ...
  /* Initialize Sizes */
  coupled_tanks_M->Sizes.numContStates = (3);/* Number of continuous states */
  ...
  coupled_tanks_M->Sizes.numBlocks = (44);/* Number of blocks */
  coupled_tanks_M->Sizes.numBlockIO = (9);/* Number of block outputs */
  coupled_tanks_M->Sizes.numBlockPrms = (130);/* Sum of parameter "widths" */
