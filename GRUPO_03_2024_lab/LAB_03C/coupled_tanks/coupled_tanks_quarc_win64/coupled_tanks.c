 ...
 * C source code generated on : Fri Nov 29 17:47:12 2024
 *
...
void coupled_tanks_output(void)
{
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    /* Gain: '<Root>/Kp' */
    coupled_tanks_B.Kp = coupled_tanks_P.Kp_Gain * coupled_tanks_B.Sum;
  }
  ...
  if ((coupled_tanks_DW.TimeStampA >= coupled_tanks_M->Timing.t[0]) &&
    ...
    rtb_Add_d = coupled_tanks_DW.TimeStampA;
    ...
    if (coupled_tanks_DW.TimeStampA < coupled_tanks_DW.TimeStampB) {
      if (coupled_tanks_DW.TimeStampB < coupled_tanks_M->Timing.t[0]) {
        rtb_Add_d = coupled_tanks_DW.TimeStampB;
    ...
    rtb_Add_d = (coupled_tanks_B.Sum - *lastU) / (coupled_tanks_M->Timing.t[0] -
      rtb_Add_d);
  }
   ...
   */
  rtb_Add_d = (coupled_tanks_P.kd_Gain * rtb_Add_d + coupled_tanks_B.Kp) +
    coupled_tanks_X.Integrator1_CSTATE;
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
  ...
  if (rtb_Add_d > coupled_tanks_P.Saturation_UpperSat) {
    rtb_Saturation = coupled_tanks_P.Saturation_UpperSat;
  } else if (rtb_Add_d < coupled_tanks_P.Saturation_LowerSat) {
    rtb_Saturation = coupled_tanks_P.Saturation_LowerSat;
  } else {
    rtb_Saturation = rtb_Add_d;
  }
  ...
  switch ((int32_T)((coupled_tanks_P.OpNormal_Value - (real_T)
                     (coupled_tanks_B.Compare ? rtb_Add_d <
                      coupled_tanks_P.Constant_Value_k : 0)) - (real_T)
                    (rtb_Add_d >= coupled_tanks_P.CompareToConstant2_const ?
                     (int32_T)coupled_tanks_B.Compare_f : 0) * (real_T)
   ...
   default:
    coupled_tanks_B.MultiportSwitch = rtb_Saturation;
    break;
  }
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    /* Gain: '<Root>/ki  ' */
    coupled_tanks_B.ki = coupled_tanks_P.ki_Gain * coupled_tanks_B.Sum;
  }
   ...
   *  Sum: '<S2>/Sum'
   */
  coupled_tanks_B.Sum1 = (rtb_Saturation - rtb_Add_d) * coupled_tanks_P.N_Gain +
    coupled_tanks_B.ki;
}
...
void coupled_tanks_derivatives(void)
{
  ...
  _rtXdot->Integrator1_CSTATE = coupled_tanks_B.Sum1;
}
...
void coupled_tanks_initialize(void)
{
  ...
  coupled_tanks_X.Integrator1_CSTATE = coupled_tanks_P.Integrator1_IC;
}
...
RT_MODEL_coupled_tanks_T *coupled_tanks(void)
{
  ...
  /* External mode info */
  coupled_tanks_M->Sizes.checksums[0] = (3314581998U);
  coupled_tanks_M->Sizes.checksums[1] = (1198182878U);
  coupled_tanks_M->Sizes.checksums[2] = (3345787276U);
  coupled_tanks_M->Sizes.checksums[3] = (1883205001U);
...

  {
    ...
    coupled_tanks_B.Kp = 0.0;
    coupled_tanks_B.MultiportSwitch = 0.0;
    coupled_tanks_B.ki = 0.0;
    coupled_tanks_B.Sum1 = 0.0;
  }
  ...
  coupled_tanks_M->Sizes.numBlocks = (43);/* Number of blocks */
  coupled_tanks_M->Sizes.numBlockIO = (10);/* Number of block outputs */
  coupled_tanks_M->Sizes.numBlockPrms = (119);/* Sum of parameter "widths" */
