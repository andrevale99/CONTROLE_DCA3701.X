 ...
 * C source code generated on : Fri Oct 25 16:16:11 2024
 *
...
void coupled_tanks_output(void)
{
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    coupled_tanks_B.SetPoint = coupled_tanks_P.Constant_Value;
  ...
  if ((coupled_tanks_DW.TimeStampA >= coupled_tanks_M->Timing.t[0]) &&
    ...
    rtb_Add = coupled_tanks_DW.TimeStampA;
    ...
    if (coupled_tanks_DW.TimeStampA < coupled_tanks_DW.TimeStampB) {
      if (coupled_tanks_DW.TimeStampB < coupled_tanks_M->Timing.t[0]) {
        rtb_Add = coupled_tanks_DW.TimeStampB;
    ...
    rtb_Add = (coupled_tanks_B.Sum - *lastU) / (coupled_tanks_M->Timing.t[0] -
      rtb_Add);
  }
   ...
   */
  rtb_Add = (coupled_tanks_P.Ki_Gain * coupled_tanks_X.Integrator_CSTATE +
             coupled_tanks_B.Kp) + coupled_tanks_P.Kd_Gain * rtb_Add;

  /* Saturate: '<Root>/Saturation' */
  if (rtb_Add > coupled_tanks_P.Saturation_UpperSat) {
    coupled_tanks_B.Saturation = coupled_tanks_P.Saturation_UpperSat;
  } else if (rtb_Add < coupled_tanks_P.Saturation_LowerSat) {
    coupled_tanks_B.Saturation = coupled_tanks_P.Saturation_LowerSat;
  } else {
    coupled_tanks_B.Saturation = rtb_Add;
  }
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    /* S-Function Block: coupled_tanks/PLANTA/HIL Write Analog (hil_write_analog_block) */
    {
      ...
      result = hil_write_analog(coupled_tanks_DW.HILInitialize_Card,
        &coupled_tanks_P.HILWriteAnalog_channels, 1, &coupled_tanks_B.Saturation);
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(coupled_tanks_M, _rt_error_message);
...
RT_MODEL_coupled_tanks_T *coupled_tanks(void)
{
  ...
  /* External mode info */
  coupled_tanks_M->Sizes.checksums[0] = (1703216397U);
  coupled_tanks_M->Sizes.checksums[1] = (4074917980U);
  coupled_tanks_M->Sizes.checksums[2] = (3880051568U);
  coupled_tanks_M->Sizes.checksums[3] = (2984519234U);
...

  {
    ...
    coupled_tanks_B.Saturation = 0.0;
  }
  ...
  coupled_tanks_M->Sizes.numBlocks = (17);/* Number of blocks */
  coupled_tanks_M->Sizes.numBlockIO = (5);/* Number of block outputs */
  coupled_tanks_M->Sizes.numBlockPrms = (107);/* Sum of parameter "widths" */
